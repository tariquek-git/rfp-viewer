...
const EMPTY_KB: KnowledgeBase = {
  companyFacts: "", keyMetrics: "", differentiators: "", competitivePositioning: "", lastUpdated: 0,
};

const EMPTY_PRICING: PricingModel = { lineItems: [], implementationFee: 0, annualRecurring: 0, currency: "USD", lastUpdated: 0 };
...
export function useRFPState() {
      ...
      .then((d: RFPData) => {
        ...
        if (saved) try {
          ...
          if (edits.questions) {
            loadedData = {
              ...edits,
              questions: edits.questions.map((q: Question) => ({ ...q, status: q.status || "draft" })),
            };
          }
        } catch { /* ignore */ }
        ...
        try { const v = localStorage.getItem("rfp-cell-history"); if (v) setCellHistory(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-global-rules"); if (v) setGlobalRules(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-validation-rules"); if (v) setValidationRules(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-feedback"); if (v) setFeedbackItems(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-versions"); if (v) setVersions(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-knowledge-base"); if (v) setKnowledgeBase(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-pricing"); if (v) setPricingModel(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-win-themes"); if (v) setWinThemes(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-milestones"); if (v) setMilestones(JSON.parse(v)); } catch { /* */ }
        try { const v = localStorage.getItem("rfp-slas"); if (v) setSLACommitments(JSON.parse(v)); } catch { /* */ }
  ...
  const saveVersion = useCallback((label?: string) => {
    if (!data) return;
    const v: Version = { label: label || `v${versions.length + 1}`, timestamp: Date.now(), data: JSON.parse(JSON.stringify(data)) };
  ...
  // === Cell History ===
  const addCellHistory = useCallback((ref: string, field: string, value: string, source: CellHistoryEntry["source"]) => {
    setCellHistory((prev) => {
      const key = `${ref}:${field}`;
  ...
  // === Status ===
  const cycleStatus = useCallback((ref: string) => {
    if (!data) return;
    const q = data.questions.find((q) => q.ref === ref);
    if (!q) return;
  ...
  const bulkSetStatus = useCallback((status: Question["status"]) => {
    if (!data) return;
    ...
    if (validationRules.filter(r => r.type === "validation").length > 0) {
      try {
        const valRes = await fetch("/api/validate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text, validationRules: validationRules.filter(r => r.type === "validation"), question, knowledgeBase }),
        });
        const valData = await valRes.json();
        setPendingDiffs(prev => ({
          ...prev,
          [key]: { ref: question.ref, field, original, suggested: text, diff, timestamp: Date.now(), validationResults: valData.results },
        }));
      } catch {
        setPendingDiffs(prev => ({
          ...prev,
          [key]: { ref: question.ref, field, original, suggested: text, diff, timestamp: Date.now() },
        }));
      }
    } else {
      setPendingDiffs(prev => ({
  ...
  // === Diff Accept/Reject ===
  const acceptDiff = useCallback((key: string) => {
    ...
    setPendingDiffs(prev => { const next = { ...prev }; delete next[key]; return next; });
    addToast("success", `Accepted AI rewrite for ${diff.field}`);
  }, [pendingDiffs, data, addCellHistory, updateQuestion, addToast]);

  const rejectDiff = useCallback((key: string) => {
    const diff = pendingDiffs[key];
    if (!diff) return;
    setCellHistory(prev => {
      const hKey = `${diff.ref}:${diff.field}`;
      return { ...prev, [hKey]: [...(prev[hKey] || []), { value: diff.suggested, timestamp: Date.now(), source: "ai" as const, rejected: true }] };
    });
    setPendingDiffs(prev => { const next = { ...prev }; delete next[key]; return next; });
    addToast("info", "Rejected AI suggestion");
  }, [pendingDiffs, addToast]);

  const acceptEditedDiff = useCallback((key: string, editedText: string) => {
    const diff = pendingDiffs[key];
    if (!diff || !data) return;
    const q = data.questions.find(q => q.ref === diff.ref);
    if (!q) return;
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (search) {
      ...
      qs = qs.filter((q) =>
        q.ref.toLowerCase().includes(s) || q.topic.toLowerCase().includes(s) ||
        q.requirement.toLowerCase().includes(s) || q.bullet.toLowerCase().includes(s) ||
    ...
    return qs;
  }, [data, activeCategory, search, confidenceFilter, compliantFilter, deliveryFilter, statusFilter, sortConfig]);

  // === Exports ===
  const handleExportCSV = useCallback(() => {
    if (!data) return;
    const headers = ["#", "Reference", "Category", "Topic", "BSB Requirement", "Response (Bullet)", "Response (Paragraph)", "Confidence", "Compliant", "Status", "Committee Score"];
    const rows = filteredQuestions.map((q) => [q.number, q.ref, q.category, q.topic, q.requirement, q.bullet, q.paragraph, q.confidence, q.compliant, q.status, q.committee_score]);
  ...
  const categoryStats = useMemo(() => {
    ...
    stats["All"] = { total: data.questions.length, approved: data.questions.filter(q => q.status === "approved").length };
  ...
  const updateSLAs = useCallback((s: SLACommitment[]) => { setSLACommitments(s); setHasUnsaved(true); }, []);

  return {
    // Core
    data, loading, activeTab, setActiveTab, activeCategory, setActiveCategory,
    // Filters
    search, setSearch, confidenceFilter, setConfidenceFilter, compliantFilter, setCompliantFilter,
    deliveryFilter, setDeliveryFilter, statusFilter, setStatusFilter, coverageFilter, setCoverageFilter,
    showRules, setShowRules, showFilters, setShowFilters,
    // Selection
    selectedQuestion, setSelectedQuestion, selectedRows, toggleRow, selectAllFiltered,
    // Sort
    sortConfig, setSortConfig,
    // Data
    filteredQuestions, categoryStats, statusCounts, unresolvedFeedback,
    // Persistence
    cellHistory, hasUnsaved, globalRules, setGlobalRules: (r: string[]) => { setGlobalRules(r); setHasUnsaved(true); },
    validationRules, updateValidationRules,
    feedbackItems, versions,
    knowledgeBase, updateKnowledgeBase,
    // Actions
    handleSave, saveToLocal, saveVersion, handleCellEdit, updateQuestion,
    handleAiRewrite, handleAddFeedback, handleResolveFeedback,
    handleExportCSV, handleExportJSON, resetFilters,
    getConfidenceColor, addCellHistory, cycleStatus, bulkSetStatus,
    // Diff
    pendingDiffs, acceptDiff, rejectDiff, acceptEditedDiff,
    // Modals
