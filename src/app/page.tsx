...
export default function Home() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white text-gray-900">
      ...
      <header className="border-b border-gray-200 px-4 py-2 flex items-center justify-between flex-shrink-0 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {state.hasUnsaved ? (
                <span className="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-medium">Unsaved</span>
              ) : lastSaved && (
        ...
        <div className="flex items-center gap-1">
          {NAV_GROUPS.map((group, gi) => (
            <div key={group.label} className="flex items-center">
              ...
              <span className="text-[8px] text-gray-400 uppercase tracking-wider font-semibold mr-1 hidden lg:inline">{group.label}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => state.setShowWinThemes(!state.showWinThemes)}
            className={`p-1.5 rounded-md ${state.showWinThemes ? "text-violet-600 bg-violet-50" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100"}`} title="Win Themes">
            <Target size={14} />
          </button>
          <button onClick={() => state.setShowRules(!state.showRules)}
            className={`p-1.5 rounded-md ${state.showRules ? "text-blue-600 bg-blue-50" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100"}`} title="Rules">
            <BookOpen size={14} />
          </button>
          <button onClick={() => state.setShowChecklist(true)} className="p-1.5 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100" title="Submission Checklist">
            <ClipboardList size={14} />
          </button>
          <button onClick={() => state.setShowTemplates(true)} className="p-1.5 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100" title="Templates">
            <FileStack size={14} />
          </button>
          <button onClick={() => setShowShortcuts(true)} className="p-1.5 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100" title="Shortcuts (?)">
            <Keyboard size={14} />
          </button>
        </div>
      </header>
          ...
          {["All", ...state.data.categories].map((cat) => {
            ...
            return (
              <button key={cat} onClick={() => state.setActiveCategory(cat)}
                ...
                {catStat && catStat.total > 0 && <span className={`ml-0.5 text-[8px] ${isActive ? "text-blue-200" : "text-gray-300"}`}>({catStat.approved}/{catStat.total})</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="border-b border-gray-200 px-6 py-2 flex-shrink-0 bg-white space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={state.handleExportCSV} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50">
                <Download size={11} /> CSV
              </button>
              <button onClick={state.handleExportJSON} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50">
                <FileJson size={11} /> JSON
              </button>

              <div className="w-px h-4 bg-gray-200 mx-0.5" />
              <button onClick={state.handlePushToCloud} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50" title="Push to Supabase">
                <CloudUpload size={11} /> Push
              </button>
              <button onClick={state.handlePullFromCloud} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50" title="Pull from Supabase">
                <CloudDownload size={11} /> Pull
              </button>
              <button onClick={() => state.saveVersion()} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50">
                <History size={11} /> v{state.versions.length + 1}
              </button>
              {state.versions.length > 0 && (
                <button onClick={() => state.setShowVersionCompare(true)} className="flex items-center border border-gray-200 text-gray-600 p-1.5 rounded-lg text-[11px] hover:bg-gray-50" title="Compare versions">
                  <GitCompareArrows size={11} />
                </button>
              )}

              <div className="w-px h-4 bg-gray-200 mx-0.5" />
              <button onClick={handleConsistencyCheck} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50" title="Consistency Check">
                <Scan size={11} /> Check
              </button>
              <button onClick={() => state.setShowNarrativeAudit(true)} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50" title="Narrative Audit">
                <BookText size={11} /> Audit
              </button>
              <button onClick={() => state.setShowSummary(true)} className="flex items-center gap-1 border border-gray-200 text-gray-600 px-2 py-1.5 rounded-lg text-[11px] font-medium hover:bg-gray-50">
              ...
              <button className="flex items-center gap-1 bg-gradient-to-r from-violet-600 to-purple-600 text-white px-2.5 py-1.5 rounded-lg text-[11px] font-medium hover:from-violet-700 hover:to-purple-700 shadow-sm">
                <Sparkles size={11} /> AI Rewrite <span className="bg-white/20 px-1 rounded text-[9px]">{state.data.stats.yellow + state.data.stats.red}</span>
              </button>
            </div>
          </div>
      ...
      {state.showVersionCompare && state.versions.length > 0 && <VersionCompare versions={state.versions} currentQuestions={state.data?.questions || []} onClose={() => state.setShowVersionCompare(false)} />}
      {state.showTemplates && state.data && <TemplateManager currentData={state.data} onLoadTemplate={state.loadTemplateData} onClose={() => state.setShowTemplates(false)} addToast={state.addToast} />}
      ...
      <footer className="border-t border-gray-200 px-6 py-1.5 text-[10px] text-gray-400 flex justify-between flex-shrink-0 bg-gray-50/50">
        <div className="flex items-center gap-3">
          <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-[9px] font-mono border border-gray-200">⌘S</kbd> save</span>
          <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-[9px] font-mono border border-gray-200">?</kbd> shortcuts</span>
