...
const TYPES: PricingLineItem['type'][] = [
  'one-time',
  'recurring',
  'per-transaction',
  'volume-tiered',
...
export default function PricingView({ pricing, onUpdate }: PricingViewProps) {
  const [newItem, setNewItem] = useState<Partial<PricingLineItem>>({
    category: 'Platform',
    type: 'recurring',
    amount: 0,
    unit: 'per month',
  ...
  const removeItem = useCallback(
    (id: string) => {
      onUpdate({
        ...pricing,
        lineItems: pricing.lineItems.filter((i) => i.id !== id),
  ...
  const totalOneTime = pricing.lineItems
    .filter((i) => i.type === 'one-time')
    .reduce((s, i) => s + i.amount, 0);
  const totalRecurring = pricing.lineItems
    .filter((i) => i.type === 'recurring')
    .reduce((s, i) => s + i.amount, 0);
  const totalPerTx = pricing.lineItems
    .filter((i) => i.type === 'per-transaction')
    .reduce((s, i) => s + i.amount, 0);
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Structured cost breakdown for BSB procurement evaluation
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: '3-Year TCO (est.)', value: tco3yr, color: 'text-gray-900' },
          ].map((card, i) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    ...
                    <th className="text-right px-4 py-2 text-xs font-semibold text-gray-500 uppercase w-28">
                      Amount
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase w-24">
                      Unit
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase">
                      Notes
        ...
        {/* Add new line item */}
        <div className="bg-white border border-dashed border-gray-300 rounded-xl p-4 mt-4">
          ...
          <div className="grid grid-cols-6 gap-2">
            ...
            <input
              ...
              className="border border-gray-200 rounded-lg px-2 py-1.5 text-xs col-span-2"
            />
            <input
              type="number"
              placeholder="Amount"
