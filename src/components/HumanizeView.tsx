...
export default function HumanizeView({ questions, onUpdateQuestion, addToast }: HumanizeViewProps) {
  ...
  const stats = useMemo(() => {
    const all = detected.map(d => selectedField === "bullet" ? d.bulletDetect : d.paragraphDetect);
    return {
    ...
    for (let i = 0; i < toProcess.length; i++) {
      ...
      try {
        const res = await fetch("/api/humanize", {
          ...
          body: JSON.stringify({ text, triggers: detect.triggers, context: `${item.question.category} - ${item.question.topic}` }),
        });
        const data = await res.json();
        if (data.text) {
          const humanizedDetect = detectAIWriting(data.text);
          setResults(prev => [...prev, {
            ref: item.ref, field: selectedField, original: text, humanized: data.text,
            originalDetect: detect, humanizedDetect, accepted: false,
          }]);
        }
      } catch { /* skip failures */ }
    ...
    <div className="overflow-auto h-full p-6 bg-gray-50/30 dark:bg-gray-900/30">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Compare original vs humanized responses side by side</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <select value={selectedField} onChange={e => setSelectedField(e.target.value as "bullet" | "paragraph")}
            ...
            <select value={filterLevel} onChange={e => setFilterLevel(e.target.value as "all" | "high" | "medium")}
              className="border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1.5 text-xs bg-white dark:bg-gray-800">
        ...
        <div className="flex items-center gap-2 mb-4">
          <button onClick={handleBatchHumanize} disabled={processing || filtered.length === 0}
            ...
            <Sparkles size={13} /> {processing ? `Humanizing ${progress.current}/${progress.total}...` : `Humanize ${Math.min(filtered.length, 20)} Responses`}
          </button>
          ...
          {processing && (
            <div className="flex-1 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${(progress.current / progress.total) * 100}%` }} />
            </div>
          )}
        </div>
        ...
        {results.length > 0 && (
          <div className="space-y-4 mb-8">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Results — Original vs Humanized</h3>
            {results.map((r, idx) => (
              <div key={r.ref} className={`border rounded-xl overflow-hidden ${r.accepted ? "border-emerald-200 dark:border-emerald-800 bg-emerald-50/30 dark:bg-emerald-900/5" : "border-gray-200 dark:border-gray-700"}`}>
                <div className="flex items-center justify-between px-4 py-2 bg-gray-50 dark:bg-gray-800 border-b dark:border-gray-700">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[9px] px-1.5 py-0.5 rounded border font-semibold ${aiDetectClasses(r.originalDetect.level)}`}>
                      Before: {aiDetectLabel(r.originalDetect.level)} ({r.originalDetect.score})
                    </span>
                    <span className="text-gray-400">→</span>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded border font-semibold ${aiDetectClasses(r.humanizedDetect.level)}`}>
                      After: {aiDetectLabel(r.humanizedDetect.level)} ({r.humanizedDetect.score})
                    </span>
                  </div>
                  {!r.accepted ? (
                    <button onClick={() => acceptResult(idx)}
                    ...
                    <span className="text-xs text-emerald-600 font-medium flex items-center gap-1"><Check size={11} /> Applied</span>
                  )}
                </div>
                <div className="grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-700">
                  <div className="p-4">
                    <div className="text-[10px] font-semibold text-red-500 uppercase mb-2">Original</div>
                    <div className="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed max-h-48 overflow-auto">
                      {r.original.slice(0, 800)}{r.original.length > 800 ? "..." : ""}
                    </div>
                  </div>
                  <div className="p-4">
                    <div className="text-[10px] font-semibold text-emerald-600 uppercase mb-2">Humanized</div>
                    <div className="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed max-h-48 overflow-auto">
                      {r.humanized.slice(0, 800)}{r.humanized.length > 800 ? "..." : ""}
                    </div>
                  </div>
                </div>
                {r.originalDetect.triggers.length > 0 && (
                  <div className="px-4 py-2 bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-700 text-[10px] text-gray-400">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-750 border-b dark:border-gray-700">
                  ...
                  <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase">AI Score</th>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase">Level</th>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase">Triggers</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {filtered.map(d => {
                  const detect = selectedField === "bullet" ? d.bulletDetect : d.paragraphDetect;
                  return (
                    <tr key={d.ref} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                      <td className="px-4 py-2 font-mono text-xs text-blue-600">{d.ref}</td>
                      <td className="px-4 py-2 text-xs text-gray-600 dark:text-gray-400">{d.question.category}</td>
                      <td className="px-4 py-2 text-xs font-bold">{detect.score}</td>
                      <td className="px-4 py-2">
                        <span className={`text-[9px] px-1.5 py-0.5 rounded border font-semibold ${aiDetectClasses(detect.level)}`}>
                          {aiDetectLabel(detect.level)}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-[10px] text-gray-400">{detect.triggers.slice(0, 3).join(", ")}</td>
                    </tr>
                  );
                })}
              </tbody>
