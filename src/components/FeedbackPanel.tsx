...
export default function FeedbackPanel({ question, feedbackItems, onAddFeedback, onResolveFeedback }: FeedbackPanelProps) {
    ...
    <div className="border-t pt-4 mt-4">
      ...
      {myFeedback.length > 0 && (
        <div className="space-y-2 mb-4">
          {myFeedback.map((f) => (
            <div key={f.timestamp} className={`border rounded p-3 text-sm ${f.resolved ? "bg-gray-50 opacity-60" : "bg-yellow-50 border-yellow-200"}`}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!f.resolved && (
                    <button onClick={() => onResolveFeedback(f.ref, f.timestamp)} className="text-xs text-green-600 hover:underline">resolve</button>
                  )}
