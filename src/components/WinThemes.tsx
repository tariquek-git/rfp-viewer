  ...
  const themeCoverage = useMemo(() => {
    return themes.map((theme) => {
      ...
      return {
        ...
        pct: Math.round((covered.length / questions.length) * 100),
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full sm:w-[480px] bg-white border-l shadow-2xl z-30 flex flex-col panel-slide-in">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Track narrative consistency across all responses
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
          ...
          {themeCoverage.map(({ theme, covered, total, pct }) => (
            <div key={theme.id} className="border border-gray-200 rounded-xl p-4 group">
              ...
              <div className="flex flex-wrap gap-1 max-h-20 overflow-auto">
                ...
                {questionsWithNoTheme.length > 20 && (
                  <span className="text-[10px] text-amber-600">
                    +{questionsWithNoTheme.length - 20} more
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="border border-gray-200 rounded-xl p-4">
            ...
            <input
              ...
              placeholder='e.g. "Mobile-first platform"'
              className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            />
            <textarea
              value={newDesc}
              ...
              rows={2}
              className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none"
            />
            <button
              onClick={addTheme}
