...
function ToastItem({ toast, onRemove }: { toast: ToastMessage; onRemove: () => void }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => { setExiting(true); setTimeout(onRemove, 300); }, 4000);
  ...
  return (
    ...
    >
      ...
      <button onClick={() => { setExiting(true); setTimeout(onRemove, 300); }} className="opacity-50 hover:opacity-100">
        <X size={14} />
      </button>
    </div>
  );
