...
export default function SLAView({ slas, onUpdate }: SLAViewProps) {
  ...
  const grouped = SLA_CATEGORIES.reduce(
    (acc, cat) => {
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Service Level Agreements and performance commitments for BSB
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm text-center">
            <div className="text-2xl font-bold text-gray-900">{slas.length}</div>
            <div className="text-xs text-gray-500">Total SLAs</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm text-center">
            <div className="text-2xl font-bold text-emerald-600">
              {slas.filter((s) => s.currentPerformance).length}
            </div>
            <div className="text-xs text-gray-500">With Performance Data</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm text-center">
            <div className="text-2xl font-bold text-amber-600">
              {slas.filter((s) => s.penalty).length}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    ...
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase w-28">
                      Target
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase">
                      Measurement
                ...
                <tbody className="divide-y divide-gray-100">
                  {items.map((sla) => (
                    <tr key={sla.id} className="group hover:bg-gray-50">
                      <td className="px-4 py-2">
                        <input
                          value={sla.metric}
                          onChange={(e) => updateSLA(sla.id, 'metric', e.target.value)}
                      ...
                      <td className="px-4 py-2">
                        <input
                          value={sla.target}
                          onChange={(e) => updateSLA(sla.id, 'target', e.target.value)}
                          className="w-full bg-transparent border-0 text-sm font-mono text-emerald-600 focus:outline-none"
                          placeholder="99.9%"
                        />
                      </td>
                      <td className="px-4 py-2">
                        <input
                          ...
                          onChange={(e) => updateSLA(sla.id, 'measurement', e.target.value)}
                          className="w-full bg-transparent border-0 text-xs text-gray-500 focus:outline-none"
                      ...
                      <td className="px-4 py-2">
                        <input
                          value={sla.penalty}
                          onChange={(e) => updateSLA(sla.id, 'penalty', e.target.value)}
                          className="w-full bg-transparent border-0 text-xs text-red-500 focus:outline-none"
                          placeholder="Credit/penalty..."
                        />
                      </td>
                      <td className="px-4 py-2">
                        <input
                          value={sla.currentPerformance}
                          onChange={(e) => updateSLA(sla.id, 'currentPerformance', e.target.value)}
                          className="w-full bg-transparent border-0 text-sm font-mono focus:outline-none"
                          placeholder="—"
                        />
                      </td>
                      <td className="px-2">
        ...
        {slas.length === 0 && (
          <div className="bg-white border-2 border-dashed border-gray-200 rounded-xl p-12 text-center">
            ...
            <p className="text-xs text-gray-400">
              Add uptime, response time, and processing commitments
            </p>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
              ...
              <div className="space-y-3">
                ...
                <input
                  placeholder="Metric (e.g. Platform Uptime)"
                  value={draft.metric || ''}
                  onChange={(e) => setDraft({ ...draft, metric: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm"
                />
                <input
                  placeholder="Target (e.g. 99.95%)"
                  value={draft.target || ''}
