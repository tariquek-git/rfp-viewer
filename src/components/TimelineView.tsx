...
const PHASES = ["Discovery", "Configuration", "Integration", "Testing", "Pilot", "Training", "Rollout", "Post-Launch"];
const STATUSES: TimelineMilestone["status"][] = ["not-started", "in-progress", "complete"];
const STATUS_COLORS = {
  "not-started": "bg-gray-200 dark:bg-gray-600",
...
export default function TimelineView({ milestones, onUpdate }: TimelineViewProps) {
  ...
  const [draft, setDraft] = useState<Partial<TimelineMilestone>>({ phase: "Discovery", startWeek: 1, durationWeeks: 2, status: "not-started" });
  ...
  const phaseColors: Record<string, string> = {
    Discovery: "bg-blue-400", Configuration: "bg-violet-400", Integration: "bg-indigo-400",
    Testing: "bg-amber-400", Pilot: "bg-orange-400", Training: "bg-teal-400",
    Rollout: "bg-emerald-400", "Post-Launch": "bg-gray-400",
  };
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm mb-6">
          ...
          <div className="flex items-center mb-4 pl-48">
            {Array.from({ length: totalWeeks }, (_, i) => (
              <div key={i} className="flex-1 text-center text-[9px] text-gray-400 font-mono">W{i + 1}</div>
            ))}
          </div>
              ...
              <div key={m.id} className="flex items-center group">
                ...
                <div className="flex-1 relative h-7">
                  <div className="absolute inset-0 flex">
                    {Array.from({ length: totalWeeks }, (_, i) => (
                      <div key={i} className="flex-1 border-l border-gray-100 dark:border-gray-700" />
                    ))}
                  </div>
                  <div
                    ...
                    <span className="text-[9px] text-white font-semibold truncate px-2">{m.durationWeeks}w</span>
                  </div>
                </div>
                <div className="w-20 flex-shrink-0 pl-2">
                  <select value={m.status} onChange={e => updateMilestone(m.id, "status", e.target.value)}
                    className="text-[10px] bg-transparent border-0 text-gray-500 focus:outline-none w-full">
                    {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <button onClick={() => removeMilestone(m.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 ml-1">
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-750 border-b dark:border-gray-700">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase w-20">Start</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase w-24">Duration</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase">Owner</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase w-24">Status</th>
              </tr>
            </thead>
              ...
              {milestones.map(m => (
                <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                  <td className="px-4 py-2">
                    <select value={m.phase} onChange={e => updateMilestone(m.id, "phase", e.target.value)}
                  ...
                  <td className="px-4 py-2">
                    <input value={m.description} onChange={e => updateMilestone(m.id, "description", e.target.value)}
                      className="w-full bg-transparent border-0 text-sm focus:outline-none" placeholder="Description..." />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" value={m.startWeek} min={1} onChange={e => updateMilestone(m.id, "startWeek", Number(e.target.value))}
                      className="w-full bg-transparent border-0 text-sm focus:outline-none font-mono" />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" value={m.durationWeeks} min={1} onChange={e => updateMilestone(m.id, "durationWeeks", Number(e.target.value))}
                      className="w-full bg-transparent border-0 text-sm focus:outline-none font-mono" />
                  </td>
                  <td className="px-4 py-2">
                    <input value={m.owner} onChange={e => updateMilestone(m.id, "owner", e.target.value)}
                      className="w-full bg-transparent border-0 text-sm focus:outline-none text-gray-500" placeholder="Team/Person" />
                  </td>
                  <td className="px-4 py-2">
                    <span className={`inline-block w-2 h-2 rounded-full mr-1.5 ${STATUS_COLORS[m.status]}`} />
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6">
              ...
              <div className="space-y-3">
                <select value={draft.phase} onChange={e => setDraft({ ...draft, phase: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm">{PHASES.map(p => <option key={p}>{p}</option>)}</select>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs text-gray-500">Start Week</label>
                    <input type="number" value={draft.startWeek} min={1} onChange={e => setDraft({ ...draft, startWeek: Number(e.target.value) })}
                      className="w-full border rounded-lg px-3 py-2 text-sm" /></div>
                  <div><label className="text-xs text-gray-500">Duration (weeks)</label>
                    <input type="number" value={draft.durationWeeks} min={1} onChange={e => setDraft({ ...draft, durationWeeks: Number(e.target.value) })}
                      className="w-full border rounded-lg px-3 py-2 text-sm" /></div>
                </div>
                <input placeholder="Owner (team/person)" value={draft.owner || ""} onChange={e => setDraft({ ...draft, owner: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
                <div className="flex gap-2 pt-2">
                  <button onClick={() => setAdding(false)} className="flex-1 border rounded-lg py-2 text-sm font-medium">Cancel</button>
                  <button onClick={addMilestone} disabled={!draft.description}
                    className="flex-1 bg-blue-600 text-white rounded-lg py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-40">Add</button>
                </div>
              </div>
            </div>
          </div>
