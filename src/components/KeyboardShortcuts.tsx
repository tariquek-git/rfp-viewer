...
export function useKeyboardShortcuts(handlers: {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (tabMap[e.key] && !e.metaKey && !e.ctrlKey) {
        handlers.onSwitchTab?.(tabMap[e.key]);
      }
    };
      ...
      >
        ...
        <div className="p-4 space-y-1">
          {SHORTCUTS.map((s, i) => (
            <div
              key={i}
              className="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              ...
              <div className="flex items-center gap-1">
                {s.keys.map((k, j) => (
                  <kbd
                    key={j}
                    className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[11px] font-mono border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 min-w-[24px] text-center"
                  >
                    {k}
                  </kbd>
                ))}
        ...
        <div className="px-6 py-3 border-t dark:border-gray-700 text-center">
          <span className="text-[10px] text-gray-400">
            Press{' '}
            <kbd className="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px] font-mono border border-gray-200 dark:border-gray-600">
              ?
            </kbd>{' '}
            to toggle
          </span>
