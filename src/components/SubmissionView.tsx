...
export default function SubmissionView({
  ...
  const handlePDFExport = async () => {
    ...
    setExporting('pdf');
    try {
      await exportToPDF(data, exportOpts);
    } catch (e) {
      console.error(e);
    }
    setExporting(null);
  ...
  return (
    <div className="overflow-auto h-full bg-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <label className="flex items-center gap-1.5 text-xs text-gray-500">
            <input
              ...
              onChange={(e) => setShowAdvisory(e.target.checked)}
              className="rounded"
            />
            Show advisory
          </label>
      ...
      <div
        ...
        className="max-w-[800px] mx-auto px-10 py-12 print:px-6 print:py-4 print:max-w-none text-[13px] leading-relaxed"
      >
        {/* === COVER PAGE === */}
        <div className="text-center mb-20 print:mb-16">
          <div className="text-[10px] uppercase tracking-[0.3em] text-gray-400 mb-6">
            Confidential
          </div>
          <h1 className="text-3xl font-bold text-gray-900 mb-3">BSB Credit Card Program</h1>
          <h2 className="text-xl text-gray-500 font-normal border-0 mb-8">
            Request for Proposal Response
          </h2>
          <div className="w-16 h-0.5 bg-blue-600 mx-auto mb-8" />
          <p className="text-base text-gray-600">
            Prepared by <strong>Brim Financial</strong>
          </p>
        ...
        {(mode === 'full' || mode === 'executive') && (
          <div className="mb-16 print:break-before-page">
            <h2 className="text-lg font-bold text-gray-900 border-b-2 border-gray-900 pb-2 mb-6">
              Executive Overview
            </h2>
            ...
            <p className="text-sm text-gray-700 leading-relaxed mb-4">
              Brim Financial is pleased to present our comprehensive response to Bangor Savings
              Bank&#39;s Credit Card Program RFP. Of the {questions.length} requirements,{' '}
              {stats.compliantY} ({Math.round((stats.compliantY / questions.length) * 100)}%) are
              fully compliant, with {stats.green} responses rated GREEN by our internal review
              committee.
            </p>

            {showAdvisory && stats.red > 0 && (
              <div className="advisory bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <h4 className="text-sm font-semibold text-blue-900 mb-2">
                  Advisory: {stats.red} Critical Gaps Identified
                </h4>
                <p className="text-xs text-blue-700 mb-3">
                  The following requirements have been flagged RED and may require strategic
                  positioning or partnership solutions:
                </p>
                <table className="risk-table w-full text-xs border-collapse">
                  ...
                  <tbody>
                    {stats.redQuestions.map((q) => (
                      <tr key={q.ref}>
                        <td className="border border-gray-200 px-3 py-1.5 font-mono">{q.ref}</td>
                        <td className="border border-gray-200 px-3 py-1.5">{q.topic}</td>
                        <td className="border border-gray-200 px-3 py-1.5 text-red-600 font-bold">
                          {q.committee_score}/10
                        </td>
                        <td className="border border-gray-200 px-3 py-1.5 text-gray-600">
                          {q.compliant === 'N' ? 'Non-compliant' : 'Partial compliance'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {/* Category scorecard */}
            <h3 className="text-sm font-bold text-gray-900 mb-3 mt-8">Category Performance</h3>
            <table className="w-full text-xs border-collapse mb-8">
              ...
              <tbody>
                {catStats.map((s) => (
                  <tr key={s.cat}>
                    <td className="border border-gray-200 px-3 py-1.5 font-medium">{s.cat}</td>
                    <td className="border border-gray-200 px-3 py-1.5 text-center">{s.count}</td>
                    <td
                      className={`border border-gray-200 px-3 py-1.5 text-center font-bold ${s.avg >= 7 ? 'text-emerald-600' : s.avg >= 5 ? 'text-amber-600' : 'text-red-600'}`}
                    >
                      {s.avg.toFixed(1)}
                    </td>
                    <td className="border border-gray-200 px-3 py-1.5 text-center text-emerald-600">
                      {s.green}
                    </td>
                    <td className="border border-gray-200 px-3 py-1.5 text-center text-amber-600">
                      {s.yellow}
                    </td>
                    <td className="border border-gray-200 px-3 py-1.5 text-center text-red-600">
                      {s.red}
                    </td>
                  </tr>
                ))}
        ...
        {mode === 'executive' && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-xs mt-2">
              Switch to &quot;Full&quot; or &quot;Responses Only&quot; for complete content.
            </p>
          </div>
        )}
            ...
            <ol className="space-y-2">
              {categories.map((cat, i) => {
                ...
                return (
                  <li
                    key={cat}
                    className="flex items-center justify-between text-sm border-b border-dotted border-gray-200 pb-1"
                  >
          ...
          categories.map((cat, catIdx) => {
            ...
            return (
              <div key={cat} className="mb-16 print:break-before-page">
                {/* Category header */}
                <div className="mb-8">
                  <h2 className="text-xl font-bold text-gray-900 border-b-2 border-gray-300 pb-2 mb-2">
                    {catIdx + 1}. {cat}
                  </h2>
                  <div className="flex items-center gap-4 text-xs text-gray-500">
                    ...
                    {(cs?.yellow || 0) > 0 && (
                      <span className="text-amber-600">{cs?.yellow} YELLOW</span>
                    )}
                ...
                {catQs.map((q) => (
                  <div key={q.ref} className="question mb-10 print:break-inside-avoid">
                    ...
                    {/* Requirement */}
                    <div className="requirement bg-gray-50 rounded-lg px-4 py-3 mb-3 border-l-3 border-l-gray-300">
                      <div className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1">
                        BSB Requirement
                      </div>
                      <p className="text-[12px] text-gray-600 leading-relaxed">{q.requirement}</p>
                    </div>

                    {/* Response */}
                    <div className="mb-2">
                      <div className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1">
                        Brim Financial Response
                      </div>
                      <div className="response text-[13px] text-gray-800 leading-[1.75] whitespace-pre-wrap">
                        {q.paragraph || q.bullet || (
                      ...
                      <div className="mt-3 space-y-1.5">
                        {q.confidence === 'RED' && (
                          <div className="bg-red-50 border border-red-200 rounded px-3 py-2 text-xs text-red-700">
                            <strong>Risk:</strong>{' '}
                            {q.committee_risk ||
                              'High risk — response needs significant strengthening'}
                          </div>
                        )}
                        ...
                        {q.rationale && (
                          <div className="text-[11px] text-gray-500 italic">
                            {q.rationale.slice(0, 200)}
                            {q.rationale.length > 200 ? '...' : ''}
                          </div>
                        )}
                        {q.pricing && (
                    ...
                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-100">
                      ...
                      <div className="flex-1 h-1 bg-gray-100 rounded-full max-w-[100px]">
                        <div
                          className={`h-1 rounded-full ${q.committee_score >= 7 ? 'bg-emerald-500' : q.committee_score >= 5 ? 'bg-amber-500' : 'bg-red-500'}`}
                          style={{ width: `${q.committee_score * 10}%` }}
                        />
                      </div>
                      <span
                        className={`text-[10px] font-bold ${q.committee_score >= 7 ? 'text-emerald-600' : q.committee_score >= 5 ? 'text-amber-600' : 'text-red-600'}`}
                      >
                        {q.committee_score}/10
                      </span>
                    </div>
        ...
        {/* === FOOTER === */}
        <div className="text-center pt-12 mt-12 border-t border-gray-200 print:break-before-page">
          ...
          <p className="text-[10px] text-gray-300 mt-3 italic">
            Responses prepared with AI assistance (Claude by Anthropic) · All content reviewed,
            verified, and approved by Brim Financial subject matter experts
          </p>
        </div>
      </div>
    </div>
