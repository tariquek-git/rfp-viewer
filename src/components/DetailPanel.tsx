...
function CollapsibleSection({ title, defaultOpen = true, children, extra }: { title: string; defaultOpen?: boolean; children: React.ReactNode; extra?: React.ReactNode }) {
...
function WordCount({ text }: { text: string }) {
  ...
  return <span className={`text-[10px] px-1.5 py-0.5 rounded ${getWordCountClasses(color)}`}>{wc} words</span>;
}
        ...
        <div className="mb-2 border rounded bg-gray-50 p-2 max-h-40 overflow-auto">
          {history.slice().reverse().map((h, i) => (
            <div key={i} className="text-xs border-b last:border-0 py-1">
              <span className={`font-medium ${h.source === "ai" ? "text-purple-600" : h.source === "ai-edited" ? "text-indigo-600" : "text-blue-600"}`}>
...
export default function DetailPanel({
  question, onClose, onSave, onAiRewrite, cellHistory, feedbackItems, onAddFeedback, onResolveFeedback,
  pendingDiffs, onAcceptDiff, onRejectDiff, onAcceptEditedDiff,
}: DetailPanelProps) {
  ...
  const handleRescore = async () => {
    ...
    try {
      ...
      if (result.committee_score) {
        setQ({ ...q, committee_score: result.committee_score, committee_review: result.committee_review || q.committee_review, committee_risk: result.committee_risk || q.committee_risk });
        setDirty(true);
      }
    } catch (e) { console.error("Re-score failed:", e); }
    setRescoring(false);
  };
    ...
    <div className="absolute right-0 top-0 bottom-0 w-[560px] bg-white border-l shadow-xl z-30 flex flex-col panel-slide-in">
      ...
      <div className="px-6 py-3 border-b flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        <span className={`text-xs px-2 py-1 rounded border font-medium capitalize ${
          ...
          q.status === "flagged" ? "bg-red-50 text-red-700 border-red-200" :
      ...
      <div className="flex-1 overflow-auto px-6 py-4 space-y-1">
        ...
        {pendingDiffs[bulletDiffKey] ? (
          <DiffView diff={pendingDiffs[bulletDiffKey]}
        ...
        {critiqueResults[`${q.ref}:bullet`] && <CritiquePanel result={critiqueResults[`${q.ref}:bullet`]} />}
        ...
        {pendingDiffs[paragraphDiffKey] ? (
          <DiffView diff={pendingDiffs[paragraphDiffKey]}
        ...
        {critiqueResults[`${q.ref}:paragraph`] && <CritiquePanel result={critiqueResults[`${q.ref}:paragraph`]} />}
        ...
        <CollapsibleSection title="Committee Review" defaultOpen={false} extra={
        ...
        <CollapsibleSection title="Row Rules" defaultOpen={true}>
          <p className="text-xs text-gray-400 mb-2">Give the AI direction for this specific question.</p>
        ...
        {/* Feedback */}
        <FeedbackPanel question={q} feedbackItems={feedbackItems} onAddFeedback={onAddFeedback} onResolveFeedback={onResolveFeedback} />
      </div>
    </div>
  );
}
