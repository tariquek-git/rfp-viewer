...
export default function RulesPanel({ onClose }: RulesPanelProps) {
  ...
  const addRule = () => {
    if (newRule.trim()) {
      setGlobalRules([...globalRules, newRule.trim()]);
      setNewRule("");
    }
  };
    ...
    <div className="absolute right-0 top-0 bottom-0 w-[450px] bg-white border-l shadow-lg z-20 flex flex-col">
      ...
      <div className="flex gap-2 px-6 pt-4">
        <button
          ...
          className={`px-3 py-1.5 rounded text-sm font-medium ${activeRuleTab === "global" ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-100"}`}
        >
          Global Rules
        </button>
        <button
          onClick={() => setActiveRuleTab("section")}
          className={`px-3 py-1.5 rounded text-sm font-medium ${activeRuleTab === "section" ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-100"}`}
        >
          Section Rules
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        {activeRuleTab === "global" && (
          <div>
            <p className="text-sm text-gray-600 mb-4">These rules apply to ALL questions across the entire RFP.</p>
            {globalRules.length === 0 && (
              <p className="text-sm text-gray-400 italic mb-4">No global rules yet. Add one below.</p>
            )}
            {globalRules.map((rule, i) => (
              <div key={i} className="border rounded p-3 mb-2 text-sm text-gray-700 flex items-start justify-between">
                <span>{rule}</span>
                <button onClick={() => setGlobalRules(globalRules.filter((_, j) => j !== i))} className="text-red-400 hover:text-red-600 ml-2 text-xs">remove</button>
              </div>
            ))}
            <div className="flex gap-2 mt-4">
              ...
              />
              <button onClick={addRule} className="bg-blue-500 text-white px-4 py-2 rounded text-sm font-medium hover:bg-blue-600 self-end">
                Add
              </button>
            </div>
          </div>
        ...
        {activeRuleTab === "section" && (
          <div>
            ...
            <p className="text-sm text-gray-500">
              Open any question&apos;s detail panel and use the &quot;Row Rules&quot; field to add guidance specific to that question. Row rules are saved with your edits.
            </p>
          </div>
        )}
      </div>
