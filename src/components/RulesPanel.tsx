...
export default function RulesPanel({ onClose, rules, onUpdateRules, validationRules, onUpdateValidationRules }: RulesPanelProps) {
  const [activeRuleTab, setActiveRuleTab] = useState<"global" | "section" | "validation">("global");
  const [newRule, setNewRule] = useState("");
  const [newValidation, setNewValidation] = useState("");

  const addRule = () => {
    if (newRule.trim()) { onUpdateRules([...rules, newRule.trim()]); setNewRule(""); }
  };
  const removeRule = (i: number) => { onUpdateRules(rules.filter((_, j) => j !== i)); };

  const addValidationRule = () => {
    if (newValidation.trim()) {
      onUpdateValidationRules([...validationRules, { id: Date.now().toString(), text: newValidation.trim(), type: "validation" }]);
      setNewValidation("");
    }
  };
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[420px] bg-white border-l shadow-2xl z-30 flex flex-col panel-slide-in">
        ...
        <div className="flex border-b border-gray-100">
          {([
            ...
            <button key={tab.key} onClick={() => setActiveRuleTab(tab.key)}
        ...
        <div className="flex-1 overflow-auto p-5">
            ...
            <div>
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                ...
                <p className="text-xs text-blue-700 leading-relaxed">
                  Click any question reference in the grid to open the detail panel. Use the <strong>Row Rules</strong> field to add guidance specific to that question.
                </p>
                <p className="text-xs text-blue-600 mt-2">Row rules are combined with global rules and feedback during AI rewrite.</p>
              </div>
            </div>
          )}
          ...
          {activeRuleTab === "validation" && (
            <div>
              <p className="text-xs text-gray-500 mb-4">Validation rules are checked <strong>after</strong> the AI generates a rewrite. Failed rules show as warnings in the diff view.</p>
              {validationRules.length === 0 ? (
                ...
                <div className="space-y-2 mb-4">
                  {validationRules.map((rule) => (
                    <div key={rule.id} className="bg-amber-50 border border-amber-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-700 flex items-start justify-between gap-3 group hover:border-amber-300">
                      <span className="leading-relaxed">{rule.text}</span>
                      <button onClick={() => removeValidationRule(rule.id)} className="text-gray-300 hover:text-red-500 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ))}
              ...
              <div className="space-y-2">
                <textarea value={newValidation} onChange={(e) => setNewValidation(e.target.value)}
                  placeholder='e.g. "Response must be under 200 words", "Must cite specific metrics"...'
                  className="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm resize-none h-20 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 placeholder:text-gray-300"
                  onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); addValidationRule(); } }} />
                <button onClick={addValidationRule} disabled={!newValidation.trim()}
