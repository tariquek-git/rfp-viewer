...
const severityConfig = {
  high: { icon: AlertOctagon, cls: "bg-red-50 border-red-200 text-red-700", badge: "bg-red-100 text-red-700" },
  medium: { icon: AlertTriangle, cls: "bg-amber-50 border-amber-200 text-amber-700", badge: "bg-amber-100 text-amber-700" },
  low: { icon: Info, cls: "bg-blue-50 border-blue-200 text-blue-700", badge: "bg-blue-100 text-blue-700" },
};
...
export default function ConsistencyResults({ issues, loading, onClose, onNavigate }: ConsistencyResultsProps) {
  return (
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            ...
            <p className="text-xs text-gray-400">{loading ? "Analyzing responses..." : `${issues.length} issues found`}</p>
          </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold ${config.badge}`}>{issue.severity.toUpperCase()}</span>
                          <span className="text-[10px] font-medium opacity-70">{typeLabels[issue.type] || issue.type}</span>
                        </div>
