...
const AI_PHRASES: Record<string, number> = {
  "comprehensive": 3, "robust": 3, "seamless": 3, "streamline": 2, "leverage": 2,
  "holistic": 3, "cutting-edge": 3, "state-of-the-art": 3, "best-in-class": 3,
  "world-class": 3, "industry-leading": 3, "innovative": 2,
  "it is worth noting": 4, "it should be noted": 4, "importantly": 2,
  "furthermore": 2, "moreover": 2, "in conclusion": 3, "in summary": 3,
  "demonstrates our": 2, "underscores": 2, "commitment to": 1,
  "designed to": 1, "tailored to": 1, "positioned to": 2,
  "ensuring that": 1, "aligns with": 1, "enhance": 1,
  "utilize": 2, "facilitate": 2, "optimize": 1, "empower": 2,
  "scalable": 1, "end-to-end": 1,
};
...
export function detectAIWriting(text: string): AIDetectResult {
  ...
  const passiveCount = (lower.match(/\b(is|are|was|were|been|being)\s+(designed|built|configured|implemented|integrated|developed|positioned|structured|tailored|crafted)\b/g) || []).length;
  if (passiveCount > 2) {
    score += passiveCount;
  ...
  const starts = sentences.map(s => s.trim().split(/\s+/).slice(0, 3).join(" ").toLowerCase()).filter(Boolean);
  if (starts.length > 3) {
  ...
  let suggestion = "";
  if (level === "high") {
    suggestion = "This response reads as AI-generated. Break up long sentences, replace em-dashes with periods, remove filler words like 'comprehensive', 'robust', 'seamless'. Use specific data points instead of superlatives.";
  } else if (level === "medium") {
    suggestion = "Some AI patterns detected. Consider shortening sentences and replacing generic phrases with specific Brim metrics.";
  }
...
export function aiDetectLabel(level: AIDetectResult["level"]): string {
  switch (level) {
    case "high": return "High AI Risk";
    case "medium": return "Medium";
    case "low": return "Human-like";
  }
}
